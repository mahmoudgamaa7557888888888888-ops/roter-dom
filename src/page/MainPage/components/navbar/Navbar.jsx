  ...
  return (
    <nav className="bg-white dark:bg-gray-800 text-gray-800 z-40 shadow-md p-4 relative">
      <div className="container mx-auto flex justify-between items-center">
        ...
        <ul className="hidden md:flex md:space-x-6 text-center">
          <Link to={`/home`}>
            <li className="p-3 text-gray-800 dark:text-white hover:text-blue-400 dark:hover:text-blue-400 cursor-pointer">
              <HomeOutlinedIcon /> الصفحه الرئيسيه
            </li>
          </Link>

          <Link to={`add-day`}>
            <li className="p-3 text-gray-800 dark:text-white hover:text-blue-400 cursor-pointer">
        ...
        {/* زر الهامبرجر - يظهر فقط في الهاتف */}
        <button
          className="flex flex-col justify-between w-8 h-6 md:hidden z-50 focus:outline-none"
          onClick={() => setOpen(!open)}
        >
          <span
            className={`block h-1 bg-gray-800 dark:bg-white rounded transform transition-transform duration-300 ease-in-out ${
              open ? "rotate-90 translate-y-2" : ""
            }`}
          ></span>

          <span
            className={`block h-1 bg-gray-800 dark:bg-white rounded transition-opacity duration-300 ease-in-out ${
          ...
          <span
            className={`block h-1 bg-gray-800 dark:bg-white rounded transform transition-transform duration-300 ease-in-out ${
              open ? "-rotate-90 -translate-y-3" : ""
            }`}
          ></span>
        </button>
      </div>
      <Sidebar open={open} onClose={()=> setOpen(false)} handelLogOut={handleLogout}/>
